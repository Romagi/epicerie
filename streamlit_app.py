...
# --- CSS PROFESSIONNEL & ACCESSIBLE (W3C) ---
st.markdown("""
<style>
    :root {
        --rouge-basque: #C41E3A;
    ...
    /* Forcer le noir pour la lisibilité humaine */
    .product-name {
    ...
    /* Boutons Ajouter - Style "Bold Noir" */
    div.stButton > button {
...
# --- BASE DE DONNÉES PRODUITS (Réintégrée pour éviter le NameError) ---
PRODUCTS = {
    "p1": {"name": "Jambon Blanc Supérieur", "shop": "Boucherie des Familles", "price": 4.50, "unit": "2 tranches", "img": "🍖", "description": "Cuit à l'ancienne, sans polyphosphates."},
    ...
    "p3": {"name": "Boîte de 6 Choux", "shop": "Maison Dune", "price": 16.00, "unit": "la boîte", "img": "🧁", "description": "Vanille, chocolat et caramel au beurre salé."},
    "p4": {"name": "Gâteau Basque Tradition", "shop": "Maison Adam", "price": 14.00, "unit": "4 parts", "img": "🥧", "description": "À la crème pâtissière, recette historique."},
    "p5": {"name": "Ossau-Iraty d'Estive", "shop": "Fromagerie Arradoy", "price": 12.50, "unit": "500g", "img": "🧀", "description": "Fromage de brebis AOP, affinage 6 mois."},
    "p6": {"name": "Filet de Merlu de Ligne", "shop": "Poissonnerie Fagoaga", "price": 18.00, "unit": "2 pers.", "img": "🐟", "description": "Pêché le matin même au port de St-Jean."},
    "p7": {"name": "Le Panier Maraîcher", "shop": "Primeur des Halles", "price": 22.00, "unit": "env. 3kg", "img": "🥦", "description": "Légumes de saison en agriculture raisonnée."},
    ...
    "p9": {"name": "Confiture Cerise Noire", "shop": "Maison Adam", "price": 6.80, "unit": "pot 350g", "img": "🍒", "description": "Idéale avec l'Ossau-Iraty."},
}

# --- HEADER & OPTIONS ---
st.markdown('<div class="hero-title">🛍️ L\'Épicerie des Halles</div>', unsafe_allow_html=True)
st.markdown("<p style='text-align:center; color:#111827; font-weight:600; margin-bottom:25px;'>Le meilleur de nos artisans, livré chez vous.</p>", unsafe_allow_html=True)

col1, col2 = st.columns(2)
with col1:
    zone = st.selectbox("📍 Livraison à :", ["Ciboure", "Saint-Jean-de-Luz", "Guéthary", "Ahetze", "Bidart"], index=1)
with col2:
    ...
    with st.expander(f"**{cat}**", expanded=True):
        for pid in pids:
            ...
            with c_info:
                ...
                st.markdown(f"<b style='color:var(--rouge-basque); font-size:1.1rem;'>{p['price']:.2f}€</b> <span style='color:#6B7280; font-size:0.85rem;'>({p['unit']})</span>", unsafe_allow_html=True)
            with c_btn:
                st.write("###") # Alignement
...
with st.sidebar:
    ...
    if not st.session_state.cart:
        ...
        livraison = 6.00
        total_final = subtotal + livraison
        
        st.write(f"Sous-total : {subtotal:.2f}€")
        st.write(f"Livraison : {livraison:.2f}€")
        ...
        st.markdown(f"""
            <a href="{wa_url}" target="_blank" style="text-decoration:none;">
                <div style="background-color:#22C55E; color:white; padding:18px; border-radius:8px; text-align:center; font-weight:900; margin-top:20px; border:1px solid #16a34a;">
...
if total_q > 0:
    ...
    <div class="floating-cart">
        🛒 {total_q} ARTICLE{'S' if total_q > 1 else ''}<br>
        <small style="font-weight:normal; font-size:0.7rem;">Voir menu ↖️</small>
    </div>
    """, unsafe_allow_html=True)
